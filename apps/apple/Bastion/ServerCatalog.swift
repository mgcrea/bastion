...
nonisolated struct BastionServer: Identifiable, Hashable {
  ...
  struct Package: Hashable {
    ...
    let vendor: Vendor
    /// Whether npm holds a SLSA build provenance attestation for the latest
    /// published version, produced by GitHub Actions **from the repository this
    /// entry links to**.
    ///
    /// The repo half is the whole point. An attestation on its own says only
    /// that some CI somewhere built the tarball, which is a thing a typosquat of
    /// a popular package can have too; matching it against `docsURL` is what
    /// makes it evidence about the entry a reader is actually looking at.
    ///
    /// A claim about the latest version, which is what `ServerInstaller` asks
    /// npm for, so the two agree. It is checked against the registry by `make
    /// provenance-check` rather than at runtime: the catalog has to render with
    /// no network, and a badge that quietly disappears offline would be read as
    /// "this package lost its provenance" rather than "the check did not run".
    ///
    /// Not defaulted, for the reason `vendor` is not. Silence must not resolve
    /// to either answer on its own.
    let provenance: Bool
  }
...
nonisolated enum ServerCatalog {
  ...
  static let all: [BastionServer] = [
    ...
    BastionServer(
      ...
      transport: .child(
        .init(
          npmName: "@mgcrea/mcp-a2a",
          binName: "a2a-mcp",
          distribution: .local,
          localPath: "mcp-a2a",
          vendor: .mgcrea,
          provenance: false)),
      docsURL: URL(string: "https://github.com/mgcrea/mcp-a2a"),
      dialect: .v2025_11_25,
      ...
      authModes: [
        .init(
          id: "shared-token",
          displayName: "Shared daemon token",
          kind: .env,
          env: ["A2A_TOKEN"],
          loginTool: nil,
          statusTool: nil,
      ...
      env: [
        .init(
          ...
          isRequired: false,
          isSecret: false,
          summary: "Where the A2A peer daemon listens. Loopback only — anything else is refused at startup. Defaults to http://127.0.0.1:41241."),
        .init(
          name: "A2A_TOKEN",
          isRequired: false,
          isSecret: true,
          summary: "Shared bearer for this machine's loopback A2A mesh. Must match the daemon's own value or every call gets 401."),
        .init(
          name: "A2A_AGENT_NAME",
          isRequired: false,
          isSecret: false,
          summary: "How this machine introduces itself in its agent card. A peer's operator reads it before delegating here."),
        .init(
          name: "A2A_PEERS",
          isRequired: false,
          isSecret: false,
          summary: "Peer BASE urls, comma-separated. Not the card path — the client appends /.well-known/agent-card.json itself."),
        .init(
          name: "A2A_MAX_WAIT_SECONDS",
          isRequired: false,
          isSecret: false,
          summary: "Long-poll ceiling. Set 150 here: Bastion's own callTimeout is 180s, and a call it kills is indistinguishable from a broken server."),
        .init(
          name: "A2A_ALLOW_WRITES",
          isRequired: false,
          isSecret: false,
          summary: "Enables delegating work to a peer and answering an inbound task — the act that commits this agent to another agent's request."),
      ]),
    ...
    BastionServer(
      ...
      env: [
        ...
        .init(
          ...
          isRequired: false,
          isSecret: false,
          summary: "NPM_OTP_MODE=totp only: which stored seed to read, through @mgcrea/mcp-totp. Defaults to npm."),
        .init(
          name: "NPM_TOTP_SECRET",
          isRequired: false,
          isSecret: true,
          summary: "NPM_OTP_MODE=totp only: an otpauth:// URI or bare base32 key, used instead of the keychain. This is npm's SECOND FACTOR, so a profile holding it alongside the token holds both halves of the account — prefer the keychain, and prefer trusted publishing over OIDC where it applies, which needs no second factor at all."),
        .init(
          name: "NPM_TOTP_KEYCHAIN_SERVICE",
          isRequired: false,
          isSecret: false,
          summary: "NPM_OTP_MODE=totp only: which keychain service the seed is stored under. Defaults to com.mgcrea.mcp-totp, which is where mcp-totp's own totp_import_uri writes it."),
        .init(
        ...
        .init(
          ...
          isRequired: false,
          isSecret: false,
          summary: "Registers the write tools: publish and unpublish, dist-tags, deprecation, package access, org and team membership, tokens, trusted-publisher changes, and the browser login that puts a session token in front of every other credential for the life of the process."),
        .init(
          name: "NPM_BIN",
          isRequired: false,
          isSecret: false,
          summary: "Path to npm's npm-cli.js, used by npm_publish to build the tarball with `npm pack`. Left unset the server finds npm beside the Node it is running under, which under Bastion is the bundled runtime. Set it to publish with a different npm than the one this app ships."),
      ]),
