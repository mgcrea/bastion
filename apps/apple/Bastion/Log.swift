...
final class LogStore {
  ...
  struct Entry: Identifiable {
    ...
    /// arguments rather than in `shopify_get_order`. The origin is searched too,
    /// so a profile name narrows the feed without needing a second control.
    ///
    ...
    func matches(_ needle: String) -> Bool {
      ...
      if origin.range(of: needle, options: options) != nil { return true }
    ...
    func matchIsHidden(_ needle: String, preview: Int) -> Bool {
      ...
      if origin.range(of: needle, options: options) != nil { return false }
