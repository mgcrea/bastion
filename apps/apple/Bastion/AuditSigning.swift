...
enum AuditSigning {
  ...
  ///
  /// Stored base64 because `CredentialStore` holds strings — the same thing the
  /// OAuth path does with a token set. Raw representation rather than a PEM: it
  /// is 32 bytes and never leaves this file.
  static func key() throws -> Curve25519.Signing.PrivateKey {
    if let stored = CredentialStore.read(.auditSigning, account: account),
    ...
    try CredentialStore.write(
      .auditSigning, account: account, value: fresh.rawRepresentation.base64EncodedString())
    return fresh
  }
  ...
  /// guards a value — the same reason `CredentialStore.accounts` exists.
  static var hasKey: Bool {
    CredentialStore.accounts(.auditSigning).contains(account)
  }

  /// Throw the key away. The next export mints a new one.
  static func forget() throws {
    try CredentialStore.delete(.auditSigning, account: account)
  }
