...
final class HostedWindow {
  ...
  func show() {
    if window == nil {
      ...
      // user resize, and nothing done here is one, so without this the bad
      // value sits in prefs forever — rediscovered and discarded on every
      ...
      // this replaces did exactly that, so a `make screenshots` run would
      // quietly resize the developer's windows to the pinned capture sizes and
      // leave them that way. Measured in cupertino, where
      // `NSWindow Frame settings-panes` was found holding exactly
