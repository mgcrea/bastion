...
/// for them.
///
/// A pane rather than a window that appears after an update. Bastion is an
...
struct WhatsNewPane: View {
  ...
  var body: some View {
    Form {
      ...
      if AppInfo.isDebugBuild, let unreleased = Changelog.unreleased {
        Section {
...
private struct ReleaseHeader: View {
  ...
  var body: some View {
    HStack(spacing: 6) {
      ...
      if release.version == AppInfo.version { Badge("installed", tint: .secondary) }
      Spacer()
      Text(formattedDate)
        .font(.caption)
