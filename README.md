...
**Bastion ships with nothing installed.** It ships with a _catalog_ of twenty-one, listed in
...
Bastion curates lightly, and only to fill the first screen. The catalog seeds twenty-one entries — ten
...
| **Catalog**             | twenty-one seeded servers, a generator, and a CI drift check                        |
...
None of the twenty-one catalog servers are modern. The ten children run an SDK whose newest protocol is
