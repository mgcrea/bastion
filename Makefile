...
api-deploy: ## Build and publish the API Worker to Cloudflare
	...
	@# fails on the first webhook. That is how this Worker spent 2026-09-07 to
	@# 09-11 returning 500 to every checkout.session.completed: migration 0002
	@# was committed, the deploy went out, `pnpm migrate` was never run, and the
	@# /health check below — a GET touching none of it — answered 200 throughout.
	@#
	...
	@CLOUDFLARE_ACCOUNT_ID=$(CF_ACCOUNT_ID) pnpm -C apps/api exec wrangler d1 migrations list bastion-licenses --remote 2>&1 \
