...
/// `license-check.swift` gives: the Xcode project has no test target.
///
/// Run with `make audit-check`.

/// Stands in for `CredentialStore`, which `AuditSigning` reaches for key
/// storage and which would drag Security.framework, `AppSupport` and the real
/// bundle identifier in behind it. Storage is not what is under test here — the
...
struct AuditCheck {
  ...
  static func main() throws {
    ...
    let manifest = Data(
      #"{"format":"bastion-audit","version":1,"records":412,"head":"c17b"}"#.utf8)
