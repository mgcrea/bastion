...
//
// The same pattern as `generate-servers.mjs`, for the same reason: the release
 ...
 *
 * Twelve lines copied from `generate-servers.mjs` rather than imported from it:
 * that module validates `servers.json` at import time and exits the process on
...
const targets = [
  {
    path: "apps/apple/Bastion/Changelog.swift",
    next: (src) =>
      region(
        src,
