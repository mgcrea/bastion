...
// icon system is built on — `bastion-mark.svg` is the only geometry — and the
// lockup is the asset that historically breaks it. Cupertino's did: the copy in
// its design canvas was hand-drawn beside the mark rather than from it, and its
// hills had been a single simplified path for two revisions before anyone
// noticed. A lockup nothing generates is a lockup nothing checks.
//
// So this does not redraw the wall, the spur, the hills, the plate gradient or
// the squircle. It embeds `design/bastion-icon.svg` whole and scales it.
// Everything that can drift is inherited, and the only numbers here are layout.
//
 ...
 * the icon, the same ratio the site's nav lockup uses.
 *
 * The gap absorbs a parity constraint: ICON + GAP + TEXT_WIDTH must be EVEN, or
 * `Math.round` splits the leftover unevenly and the two margins differ by a
 * pixel — invisible, but untestable, and the test below asserts they are equal.
 * TEXT_WIDTH is a measurement of the word, so the gap is the only free number.
 * 176 + 44 + 334 = 554. Cupertino needs 45 here for the same reason and the
 * opposite parity; do not copy either value across without re-checking the sum.
 */
const ICON = 176;
const GAP = 44;
...
/**
 * The advance of "Bastion" per em of font size, in SF Pro Display semibold,
 * tightened by the 2% tracking the wordmark carries everywhere else. Measured
 * with CoreText at 104px rather than guessed — 346.6 natural, 334.1 with the
 * tracking applied to the six interior gaps and not to the trailing edge —
 ...
 */
const WORDMARK_EM = 334 / 104;

/**
 * SF's cap height as a fraction of the em, measured (73.28 at 104). The word is
 * optically centred on its caps rather than on its baseline: "Bastion" has no
 * descender at all, so baseline-centring reads high.
 */
...
const expect = (condition, message) => {
  if (!condition) throw new Error(`bastion-lockup: ${message}`);
};
 ...
 *
 * @param {string} iconSvg contents of `design/bastion-icon.svg`
 ...
 */
export const composeLockup = (iconSvg, palette, word = "Bastion") => {
...
// that reads og:image. The card's title and description render BELOW the image
// in small grey type that truncates on a phone, so the image is the whole
// message — which is why it is a composition and not the bare icon on black.
//
...
// is already macOS-only — `make icon` needs appshot, the app needs Xcode 26, and
// the root package.json declares `"os": ["darwin"]`. Rendering this on Linux CI
// would silently swap the typeface. Nothing enforces that; it is why this
// comment is.
//
// `composeCard` does NOT fit text: both lines are a fixed size, centred, with no
// measuring and no wrap, so a line that outgrows 1200px is clipped at both ends
// in a picture nothing in CI looks at. Keep the headline short and put the
// detail in the subhead, which is set smaller and has more room.
 ...
 * so a ground quoted in appshot's terms lands here the way appshot would draw
 * it — which matters the day this repo grows framed store visuals.
 *
 ...
 * 180 and 0. `design/colors.json` uses the CSS convention for `plate` and
 * `wash`, and `composeLockup` asserts `angle === 180` on that basis. The two
 * conventions must not meet: this takes a raw appshot angle, never a palette
 * entry.
 */
 ...
 */
export const composeCard = (iconSvg, palette, { ground, headline, subhead, word = "Bastion" }) => {
  const light = palette?.colors?.paper;
  expect(light, "colors.json has no `colors.paper` to reverse the wordmark out in");
