...
// notices — and since the source is public, "anyone" includes people reading the
// algorithm rather than guessing at it.
//
...
// the signature check.
//
// What these cannot prove is that Swift agrees: they are JavaScript verifying
// JavaScript. `make license-check` compiles License.swift and runs a minted key
// through the real verifier, and that is the check that would catch a base64url
// or payload-encoding disagreement between the two.
//
...
describe("verifyKey refuses what it should", () => {
  const cases = [
    ...
    ["two segments", "bas1.abc", "expected three dot-separated parts"],
    ["an unknown prefix", valid.replace(/^bas1\./, "bas2."), "unknown key format 'bas2'"],
    ["an empty payload", "bas1..abc", "empty payload or signature"],
  ];
...
describe("the Swift twin", () => {
  // apps/apple/Bastion/License.swift makes the same four checks in the same
  ...
  // this side. So it gets asserted rather than remembered.
  const swift = readFileSync(
    join(import.meta.dirname, "../../apps/apple/Bastion/License.swift"),
    "utf8",
  );
