...
//
// The app cannot phone home. scripts/audit-listener.sh asserts Bastion binds
// loopback and nothing else, and License.swift names Updates.swift as the single
// documented exception — so a key has to prove itself from nothing but the bytes
// it carries and a public key compiled into the binary. That rules out every
// activation server and leaves exactly one shape: sign a payload here, verify it
// there, never ask anyone.
//
...
// feeds is a dozen readable lines in Gateway.swift and the source is public, so
// hardening this is unenforceable by construction against the audience it sells
// to. What this buys is a key that cannot be forged or altered — not a program
// that cannot be modified.
//
// Dependency-free (node builtins only). The format is identical to cupertino's
// but for the prefix, and deliberately so: if this ever grows a Worker issuer it
// will implement the same bytes against WebCrypto, which is why the signature
// covers the ENCODED payload rather than the parsed object. JSON key order and
// whitespace then stop being anybody's problem, in any language.
...
export const KEY_PREFIX = "bas1";

/**
 * The public half of the signing key, in base64url.
 *
 * Lives here AND in apps/apple/Bastion/License.swift, because the app cannot
 * import from this file. Two copies of one constant is a drift risk with a nasty
 * failure — every paid key refused at once — so license.test.mjs reads the Swift
 * file and asserts the two agree, rather than trusting whoever edits one to
 * remember the other.
 *
 * Not a secret — it only ever verifies. The private half lives in `.env` and
 * nowhere else in this repository, and is the one thing in this scheme that must
 * never leak and must never be lost: leaking it lets anyone issue keys, losing it
 * means no new keys can be issued for this major version ever again.
 */
export const PUBLIC_KEY = "yhb2TSXJknw4cUa74WJdXiFQyBEJA-iv4qt5icIRN_Q";
 ...
 *
 * Sortable by issue time, which is what makes Revocations.swift readable rather
 ...
 *
 * `privateKey` is PKCS#8 DER in base64 — one opaque string to paste into `.env`,
 * and the only thing that must never leak. `publicKey` is the raw 32 bytes in
 * base64url, because that is what CryptoKit's
 * `Curve25519.Signing.PublicKey(rawRepresentation:)` takes and what gets compiled
 * into the app.
 */
 ...
 * integer — see AppInfo.major, which is what the app compares against. `id` is
 * the revocation handle and the only field a refund needs.
 */
export const mint = ({
  email,
  id = ulid(),
 ...
 * mostly that one sentence. The Swift twin in apps/apple/Bastion/License.swift
